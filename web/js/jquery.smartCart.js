...
;(function ($, window, document, undefined) {
    ...
    var defaults = {
        ...
        cartItemTemplate: '<span class="list-group-item-heading" style="width: 60%;display: inline-block;">{product_name}</span>',
        cartItemQtyTemplate: '  {display_quantity}   {display_amount}',
        ...
        lang: { // Language variables
            ...
            subtotal: 'Total:',
            cartRemove: '×',
            cartEmpty: 'Panier vide!<br />Sellectionner des produits'
        },
        submitSettings: {
    ...
    $.extend(SmartCart.prototype, {
        ...
        _getProductDetails: function (elm) {
            var mi = this;
            var p = {};
            elm.children(this.options.productContainerSelector).find(this.options.productElementSelector).each(function () {
        ...
        _addUpdateCartItem: function (p) {
            ...
            if (elmMain && elmMain.length > 0) {
                ...
                elmMain.find(".sc-cart-item-amount").text(productAmount);
            } else {
                elmMain = $('<div style="padding: 5px;"></div>').addClass('sc-cart-item list-group-item');
                elmMain.append('<button type="button" class="sc-cart-remove"> <i class="fa fa-trash-o" style="    font-size: 14px;"></i> </button>');
                elmMain.attr('data-unique-key', p.unique_key);

                elmMain.append(this._formatTemplate(this.options.cartItemTemplate, p));

                var itemSummary = '  <input type="number" min="1" max="1000" class="sc-cart-item-qty" value="' + this._getValueOrEmpty(p[this.options.paramSettings.productQuantity]) + '" />';
                itemSummary += '   <span class="sc-cart-item-amount">' + productAmount + '</span></div>';
